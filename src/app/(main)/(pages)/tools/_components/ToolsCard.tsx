	...
	return (
		<Card className="flex w-full items-center justify-between ">
			<CardHeader className="flex flex-col gap-1">
				...
				<div>
					<CardTitle className="text-sm md:text-xl font-sans">{title}</CardTitle>
					<CardDescription className="text-xs md:text-[1rem] font-sans">{description}</CardDescription>
				</div>
			</CardHeader>
            <div className="flex flex-col items-center gap-1 px-3 md:p-4 font-sans">
            {connected[type] ? (
          <div className="border-bg-primary rounded-lg border-2 md:px-3 py-2 md:font-bold text-white">
            Connected
          </div>
        ) : (
          <Link
